    <script>
        ...
        async function guardarProductoBase() {
            try {
                ...
                document.querySelector('#form-producto .submit-btn').textContent = 'Guardar Producto';
        ...
        async function cargarProductos() {
            try {
                ...
                tbody.innerHTML = productos.map(p => {
                    ...
                    return `<tr><td>${p.nombre}</td><td>${p.codigo}</td><td>${p.categoria || '-'}</td><td style="color:${totalStock<5?'#e74c3c':'#333'}">${totalStock}</td><td>$${precioMin}</td><td><button class="action-btn" onclick="editarProducto(${p.id})"><i class="fas fa-edit"></i></button><button class="action-btn delete-btn" onclick="eliminarProducto(${p.id})"><i class="fas fa-trash"></i></button></td></tr>`;
                }).join('');
            } catch(e) { console.error(e); }
        }

        function editarProducto(id) { mostrarAlerta('Editar producto en desarrollo', 'warning'); }
        function eliminarProducto(id) { if(confirm('¿Eliminar producto?')) mostrarAlerta('Producto eliminado', 'success'); }
        function verVariantes(id) { mostrarAlerta('Ver variantes', 'warning'); }
        ...
        async function cargarProveedores() {
            try {
                ...
                if (!proveedores.length) { tbody.innerHTML = '<tr><td colspan="5">No hay proveedores<\/td></td>'; return; }
                document.getElementById('stats-proveedores').textContent = proveedores.length;
                tbody.innerHTML = proveedores.map(p => `<tr><td><strong>${p.nombre}</strong></td><td>${p.contacto || '-'}</td><td>${p.telefono || '-'}</td><td>${p.email || '-'}</td><td><button class="action-btn" onclick="editarProveedor(${p.id})"><i class="fas fa-edit"></i></button><button class="action-btn delete-btn" onclick="eliminarProveedor(${p.id})"><i class="fas fa-trash"></i></button></td></tr>`).join('');
            } catch(e) { console.error(e); }
        }
        ...
        function editarProveedor(id) { mostrarAlerta('Editar proveedor', 'warning'); }
        function eliminarProveedor(id) { if(confirm('¿Eliminar?')) mostrarAlerta('Proveedor eliminado', 'success'); }
        function guardarProveedor() { mostrarAlerta('Guardar proveedor', 'warning'); cerrarFormulario('proveedor'); }
        ...
        function guardarCompra() { mostrarAlerta('Guardar compra', 'warning'); cerrarFormulario('compra'); }
        function editarCompra(id) { mostrarAlerta('Editar compra', 'warning'); }
        function eliminarCompra(id) { if(confirm('¿Eliminar?')) mostrarAlerta('Compra eliminada', 'success'); }

        // Gastos
        async function cargarGastos() {
            const tbody = document.querySelector('#tabla-gastos tbody');
            if (tbody) tbody.innerHTML = '<td><td colspan="5">Cargando...<\/td></tr>';
        }
        function guardarGasto() { mostrarAlerta('Guardar gasto', 'warning'); cerrarFormulario('gasto'); }
        function editarGasto(id) { mostrarAlerta('Editar gasto', 'warning'); }
        function eliminarGasto(id) { if(confirm('¿Eliminar?')) mostrarAlerta('Gasto eliminado', 'success'); }
        ...
        function guardarPerfil() { mostrarAlerta('Guardar usuario', 'warning'); cerrarFormulario('perfil'); }
        function editarPerfil(id) { mostrarAlerta('Editar usuario', 'warning'); }
        function eliminarPerfil(id) { if(confirm('¿Eliminar?')) mostrarAlerta('Usuario eliminado', 'success'); }
        ...
        async function cargarVentas() {
            ...
            if (tbody) tbody.innerHTML = '<tr><td colspan="5">Cargando...<\/td></td>';
        }
        function verFactura(id) { mostrarAlerta('Ver factura', 'warning'); }

        // Utilitarios
        function mostrarFormulario(tipo) { document.getElementById(`form-${tipo}`)?.classList.add('active'); }
        function cerrarFormulario(tipo) { document.getElementById(`form-${tipo}`)?.classList.remove('active'); }
