...
async function verVariantes(id) {
    try {
        ...
        let modalHTML = `
            <div id="modal-variantes" style="position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.5); display:flex; align-items:center; justify-content:center; z-index:10000;">
                <div style="background:white; border-radius:25px; padding:2rem; max-width:800px; width:90%; max-height:80vh; overflow-y:auto;">
                    <h2 style="color:#ff6b6b;">${producto.nombre} - ${producto.codigo}</h2>
        `;
        
        tallas.forEach(t => {
            modalHTML += `
                <div style="margin: 1.5rem 0; border:2px solid #ffe4e9; border-radius:15px; padding:1rem;">
                    <h3 style="color:#ff6b6b;">Talla: ${t.talla}</h3>
                    <div style="display:flex; gap:1rem; flex-wrap:wrap;">
            `;
            
            t.colores?.forEach(c => {
                modalHTML += `
                    <div style="text-align:center; min-width:100px; padding:1rem; background:#fff9fc; border-radius:10px;">
                        <div style="width:40px; height:40px; background:${c.color_codigo || '#ccc'}; border-radius:50%; margin:0 auto;"></div>
                        <p><strong>${c.color_nombre}</strong></p>
                        <p>Stock: ${c.stock}</p>
                        <p>$${c.precio_venta}</p>
                    </div>
                `;
        ...
        modalHTML += `
                    <button onclick="document.getElementById('modal-variantes').remove()" style="background:#ffb6c1; color:white; border:none; padding:1rem; border-radius:50px; width:100%;">Cerrar</button>
                </div>
            </div>
        `;
